<template>
  <view class="zhangjinxi">
    ...
    <view>view组件：使用自定义{{env}}环境变量process.env.TARO_APP_NAME{{ name }}</view>
    <text>Pinia数据：{{ counter.count }}</text>
...
import Taro, { useDidShow,useRouter,useLoad,useReady, useDidHide,useUnload,usePullDownRefresh,useReachBottom,usePageScroll,useResize,useShareAppMessage,useTabItemTap,useShareTimeline,useAddToFavorites,useSaveExitState} from '@tarojs/taro'
import { ref } from 'vue'
import './index.scss'
...
export default {
  setup () {
    ...
    setTimeout(()=>Taro.setNavigationBarTitle({ title:'这是使用Taro.setNavigationBarTitle设置的title'}),3000)
